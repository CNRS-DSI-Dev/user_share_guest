...
foreach ($navItems as $item) {
    if (!in_array($item['id'], array('files', 'sharingin'))) {
        continue;
    }
    $content = '';
...
$tmpl = new OCA\User_Share_Guest\Lib\TemplateShareGuest('user_share_guest', 'index', 'user');
$tmpl->assign('usedSpacePercent', (int)$storageInfo['relative']);
$tmpl->assign('isPublic', false);
$tmpl->assign("encryptedFiles", \OCP\Util::encryptedFiles());
$tmpl->assign("mailNotificationEnabled", $config->getAppValue('core', 'shareapi_allow_mail_notification', 'no'));
$tmpl->assign("allowShareWithLink", $config->getAppValue('core', 'shareapi_allow_links', 'no'));
